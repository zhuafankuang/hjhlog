...
var config=NewConfig()  // struct *Config

//init fileMap
func InitOffsetFile(conf *Config) {
	config = conf 
	cfgs, err := ListDir(config.OffsetFileDir, "cfg")
	if err != nil {
		log.Println("read offset cfg error:", err)
	}
	for _, cfg := range cfgs {
		fi, err := os.OpenFile(config.OffsetFileDir+"/"+cfg, os.O_RDWR, 0)
		if err != nil {
...
func fileOffset(topic string, partition int32, offset int64, groupId string) {
	...
	if exist == false {
		...
		fi, err := os.Create(config.OffsetFileDir+"/" + topic + ".cfg")
		if err != nil {
